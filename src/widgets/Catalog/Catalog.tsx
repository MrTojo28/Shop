...
export function Catalog({ currentCategory, selectProductHandler }: Props) {
  const selectedCategory = Categories.filter(
    (category) => category.id === currentCategory
  );
  const categoriesList = currentCategory ? selectedCategory : Categories;
      ...
      {categoriesList.map((category) => (
        <div className="category">
          ...
          <div className="category__content">
            {Products.filter((product) => product.category === category.id).map(
              (product) => {
                return (
                  <Card
                    data={product}
